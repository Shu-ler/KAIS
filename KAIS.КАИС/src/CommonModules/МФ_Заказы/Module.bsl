...
Функция ОбновитьСписокЗаказов(Профиль) Экспорт
	...
	Пока НаборПрофилей.Следующий() Цикл
		...
		АдресРесурсаШаблон = НаборПрофилей.РесурсЗаказы;
		БезДетализации = НаборПрофилей.РежимЗаказовБезДетализации;
...
Функция ОбработатьСтраницуЗаказов(Профиль, СтраницаВHTML, Счетчик, СписокПоставщиков)

	МинимальнаяДата = ТекущаяДатаСеанса() - 5 * 24 * 60 * 60;
...
Процедура СтрокиЗаказовНаСайте(Профиль, Номер, Шифр, СписокПоставщиков, ВыборкаПоставщиков)
