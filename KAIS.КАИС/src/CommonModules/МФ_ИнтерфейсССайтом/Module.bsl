...
Процедура ПолучитьКаталогИАртикул(ИсходнаяСтрока, Артикул, Каталог)
	...
	Артикул = НормализованныйАртикул(Артикул);

КонецПроцедуры

Функция НормализованныйАртикул(ИсходныйАртикул)
	
	Удаляемые = ",./_-|\<>;:’”+`~^*=()[]{} ";
	
	Артикул = СокрЛП(ИсходныйАртикул);
	
	Для индекс = 1 По СтрДлина(Удаляемые) Цикл
		
		ИскомыйСимвол = Сред(Удаляемые, индекс, 1);
		Артикул = СтрЗаменить(Артикул, ИскомыйСимвол, "");
